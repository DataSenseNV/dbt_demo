CREATE OR REPLACE PROCEDURE "DEMO_PROC"."SET_FMC_MTD_BV_INIT_DEMO"(P_DAG_NAME VARCHAR2,
...
var HIST_INS = snowflake.createStatement( {sqlText: `
	...
	( 
		...
		WHERE  "FMCH_SRC"."SRC_BK" IN('JDE', 'SALESFORCE') AND "FMCH_SRC"."SUCCESS_FLAG" = 1
	)
	SELECT 
...
var DV_LCI_INS = snowflake.createStatement( {sqlText: `
	...
	FROM "DEMO_FMC"."FMC_LOADING_HISTORY" "FMCH_LCI"
	WHERE  "FMCH_LCI"."SRC_BK" IN('JDE', 'SALESFORCE') AND "FMCH_LCI"."SUCCESS_FLAG" = 1
	;
`} ).execute();

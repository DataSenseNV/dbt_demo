{{
	config(
		...
		alias='LKS_JDE_EMAIL_WHOISWHO',
		schema='DEMO_FL',
		unique_key = ['"LNK_EMAIL_WHOISWHO_HKEY"', '"LOAD_DATE"'],
		merge_update_columns = ['"LOAD_END_DATE"', '"DELETE_FLAG"', '"CDC_TIMESTAMP"'],
		tags=['JDEDWARDS', 'LKS_JDE_EMAIL_WHOISWHO_INCR', 'LKS_JDE_EMAIL_WHOISWHO_INIT']
	)
}}
select * from (
	WITH "END_DATING" AS 
	( 
		SELECT 
			  "LKS_TEMP_SRC_DK"."EMAIL_HKEY" AS "EMAIL_HKEY"
			, "LKS_TEMP_SRC_DK"."LOAD_DATE" AS "LOAD_DATE"
			, CASE WHEN LEAD( "LKS_TEMP_SRC_DK"."DELETE_FLAG" || "LKS_TEMP_SRC_DK"."SOURCE",1)OVER(PARTITION BY "LKS_TEMP_SRC_DK"."EMAIL_HKEY" ORDER BY "LKS_TEMP_SRC_DK"."LOAD_DATE",
				"LKS_TEMP_SRC_DK"."LOAD_CYCLE_ID")= 'Y' || 'STG' THEN CAST('Y' AS VARCHAR) ELSE "LKS_TEMP_SRC_DK"."DELETE_FLAG" END AS "DELETE_FLAG"
			, LEAD("LKS_TEMP_SRC_DK"."CDC_TIMESTAMP",1)OVER(PARTITION BY "LKS_TEMP_SRC_DK"."EMAIL_HKEY" ORDER BY "LKS_TEMP_SRC_DK"."LOAD_DATE",
				"LKS_TEMP_SRC_DK"."LOAD_CYCLE_ID") AS "CDC_TIMESTAMP"
		FROM {{ ref('JDEDWARDS_STG_LKS_JDE_EMAIL_WHOISWHO_TMP') }} "LKS_TEMP_SRC_DK"
		WHERE  "LKS_TEMP_SRC_DK"."EQUAL" = 0
	)
	, "CALC_LOAD_END_DATE" AS 
	( 
		SELECT 
			  "LKS_TEMP_SRC_US"."LNK_EMAIL_WHOISWHO_HKEY" AS "LNK_EMAIL_WHOISWHO_HKEY"
			, "LKS_TEMP_SRC_US"."LOAD_DATE" AS "LOAD_DATE"
			, "LKS_TEMP_SRC_US"."LOAD_CYCLE_ID" AS "LOAD_CYCLE_ID"
			, COALESCE(LEAD("LKS_TEMP_SRC_US"."LOAD_DATE",1)OVER(PARTITION BY "LKS_TEMP_SRC_US"."EMAIL_HKEY" ORDER BY "LKS_TEMP_SRC_US"."LOAD_DATE",
			...
			, "LKS_TEMP_SRC_US"."EARCK7" AS "EARCK7"
			, "LKS_TEMP_SRC_US"."EAIDLN" AS "EAIDLN"
			, "LKS_TEMP_SRC_US"."EAAN8" AS "EAAN8"
		FROM {{ ref('JDEDWARDS_STG_LKS_JDE_EMAIL_WHOISWHO_TMP') }} "LKS_TEMP_SRC_US"
		INNER JOIN "END_DATING" "END_DATING" ON  "LKS_TEMP_SRC_US"."EMAIL_HKEY" = "END_DATING"."EMAIL_HKEY" AND "LKS_TEMP_SRC_US"."LOAD_DATE" = "END_DATING"."LOAD_DATE"
	...
	( 
		SELECT 
			  "CALC_LOAD_END_DATE"."LNK_EMAIL_WHOISWHO_HKEY" AS "LNK_EMAIL_WHOISWHO_HKEY"
	...
	SELECT 
		  "FILTER_LOAD_END_DATE"."LNK_EMAIL_WHOISWHO_HKEY"
...
select * from (
	...
	( 
		SELECT 
			  "STG_INR_SRC"."LNK_EMAIL_WHOISWHO_HKEY" AS "LNK_EMAIL_WHOISWHO_HKEY"
	...
	SELECT 
		  "STG_SRC"."LNK_EMAIL_WHOISWHO_HKEY" AS "LNK_EMAIL_WHOISWHO_HKEY"

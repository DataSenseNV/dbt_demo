{{
	config(
		...
		alias='LKS_JDE_COMPANY_ACCOUNT_TMP',
		schema='JDEDWARDS_STG',
		tags=['JDEDWARDS', 'LKS_JDE_COMPANY_ACCOUNT_INCR']
	)
}}
select * from (
	WITH "DIST_STG" AS 
	( 
		...
		GROUP BY  "STG_DIS_SRC"."COMPANY_HKEY",  "STG_DIS_SRC"."LOAD_CYCLE_ID"
	)
	, "TEMP_TABLE_SET" AS 
	( 
		SELECT 
			  "STG_TEMP_SRC"."LNK_COMPANY_ACCOUNT_HKEY" AS "LNK_COMPANY_ACCOUNT_HKEY"
		...
		UNION 
		SELECT 
			  "LKS_SRC"."LNK_COMPANY_ACCOUNT_HKEY" AS "LNK_COMPANY_ACCOUNT_HKEY"
			...
			, "LKS_SRC"."CCCO" AS "CCCO"
			, "LKS_SRC"."CCAN8" AS "CCAN8"
		FROM {{ source('DEMO_FL', 'LKS_JDE_COMPANY_ACCOUNT') }} "LKS_SRC"
		INNER JOIN {{ source('DEMO_FL', 'LNK_COMPANY_ACCOUNT') }} "LNK_SRC" ON  "LKS_SRC"."LNK_COMPANY_ACCOUNT_HKEY" = "LNK_SRC"."LNK_COMPANY_ACCOUNT_HKEY"
		INNER JOIN "DIST_STG" "DIST_STG" ON  "LNK_SRC"."COMPANY_HKEY" = "DIST_STG"."COMPANY_HKEY"
		WHERE  "LKS_SRC"."LOAD_END_DATE" = TO_TIMESTAMP('31/12/2999 23:59:59' , 'DD/MM/YYYY HH24:MI:SS')
	)
	SELECT 
		  "TEMP_TABLE_SET"."LNK_COMPANY_ACCOUNT_HKEY" AS "LNK_COMPANY_ACCOUNT_HKEY"
		...
		, CASE WHEN "TEMP_TABLE_SET"."SOURCE" = 'STG' AND TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."ACCOUNT_HKEY")
			= LAG( TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."ACCOUNT_HKEY"),1)OVER(PARTITION BY "TEMP_TABLE_SET"."COMPANY_HKEY" ORDER BY "TEMP_TABLE_SET"."LOAD_DATE","TEMP_TABLE_SET"."ORIGIN_ID")THEN 1 ELSE 0 END AS "EQUAL"

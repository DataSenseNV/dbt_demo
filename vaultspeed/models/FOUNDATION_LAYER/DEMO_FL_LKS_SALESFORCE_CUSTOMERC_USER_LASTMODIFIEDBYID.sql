{{
	config(
		...
		alias='LKS_SALESFORCE_CUSTOMERC_USER_LASTMODIFIEDBYID',
		schema='DEMO_FL',
		unique_key = ['"LNK_CUSTOMERC_USER_LASTMODIFIEDBYID_HKEY"', '"LOAD_DATE"'],
		merge_update_columns = ['"LOAD_END_DATE"', '"DELETE_FLAG"', '"CDC_TIMESTAMP"'],
		tags=['SALESFORCE', 'LKS_SALESFORCE_CUSTOMERC_USER_LASTMODIFIEDBYID_INCR', 'LKS_SALESFORCE_CUSTOMERC_USER_LASTMODIFIEDBYID_INIT']
	)
}}
select * from (
	WITH "END_DATING" AS 
	( 
		SELECT 
			  "LKS_TEMP_SRC_DK"."CUSTOMER__C_HKEY" AS "CUSTOMER__C_HKEY"
			, "LKS_TEMP_SRC_DK"."LOAD_DATE" AS "LOAD_DATE"
			, CASE WHEN LEAD( "LKS_TEMP_SRC_DK"."DELETE_FLAG" || "LKS_TEMP_SRC_DK"."SOURCE",1)OVER(PARTITION BY "LKS_TEMP_SRC_DK"."CUSTOMER__C_HKEY" ORDER BY "LKS_TEMP_SRC_DK"."LOAD_DATE",
				"LKS_TEMP_SRC_DK"."LOAD_CYCLE_ID")= 'Y' || 'STG' THEN CAST('Y' AS VARCHAR) ELSE "LKS_TEMP_SRC_DK"."DELETE_FLAG" END AS "DELETE_FLAG"
			, LEAD("LKS_TEMP_SRC_DK"."CDC_TIMESTAMP",1)OVER(PARTITION BY "LKS_TEMP_SRC_DK"."CUSTOMER__C_HKEY" ORDER BY "LKS_TEMP_SRC_DK"."LOAD_DATE",
				"LKS_TEMP_SRC_DK"."LOAD_CYCLE_ID") AS "CDC_TIMESTAMP"
		FROM {{ ref('SALESFORCE_STG_LKS_SALESFORCE_CUSTOMERC_USER_LASTMODIFIEDBYID_TMP') }} "LKS_TEMP_SRC_DK"
		WHERE  "LKS_TEMP_SRC_DK"."EQUAL" = 0
	)
	, "CALC_LOAD_END_DATE" AS 
	( 
		SELECT 
			  "LKS_TEMP_SRC_US"."LNK_CUSTOMERC_USER_LASTMODIFIEDBYID_HKEY" AS "LNK_CUSTOMERC_USER_LASTMODIFIEDBYID_HKEY"
			, "LKS_TEMP_SRC_US"."LOAD_DATE" AS "LOAD_DATE"
			, "LKS_TEMP_SRC_US"."LOAD_CYCLE_ID" AS "LOAD_CYCLE_ID"
			, COALESCE(LEAD("LKS_TEMP_SRC_US"."LOAD_DATE",1)OVER(PARTITION BY "LKS_TEMP_SRC_US"."CUSTOMER__C_HKEY" ORDER BY "LKS_TEMP_SRC_US"."LOAD_DATE",
		...
		FROM {{ ref('SALESFORCE_STG_LKS_SALESFORCE_CUSTOMERC_USER_LASTMODIFIEDBYID_TMP') }} "LKS_TEMP_SRC_US"
		INNER JOIN "END_DATING" "END_DATING" ON  "LKS_TEMP_SRC_US"."CUSTOMER__C_HKEY" = "END_DATING"."CUSTOMER__C_HKEY" AND "LKS_TEMP_SRC_US"."LOAD_DATE" = 
	...
	( 
		SELECT 
			  "CALC_LOAD_END_DATE"."LNK_CUSTOMERC_USER_LASTMODIFIEDBYID_HKEY" AS "LNK_CUSTOMERC_USER_LASTMODIFIEDBYID_HKEY"
	...
	SELECT 
		  "FILTER_LOAD_END_DATE"."LNK_CUSTOMERC_USER_LASTMODIFIEDBYID_HKEY"
...
select * from (
	...
	( 
		SELECT 
			  "STG_INR_SRC"."LNK_CUSTOMERC_USER_LASTMODIFIEDBYID_HKEY" AS "LNK_CUSTOMERC_USER_LASTMODIFIEDBYID_HKEY"
	...
	SELECT 
		  "STG_SRC"."LNK_CUSTOMERC_USER_LASTMODIFIEDBYID_HKEY" AS "LNK_CUSTOMERC_USER_LASTMODIFIEDBYID_HKEY"

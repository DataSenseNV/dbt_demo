{{
	config(
		...
		alias='SAT_JDE_SALESORDERHEADER_TMP',
		schema='JDEDWARDS_STG',
		tags=['JDEDWARDS', 'SAT_JDE_SOHEADER_INCR']
	)
}}
select * from (
	WITH "DIST_STG" AS 
	...
	( 
		...
		UNION ALL 
		SELECT 
			  "SAT_SRC"."SALESORDERHEADER_HKEY" AS "SALESORDERHEADER_HKEY"
		...
		FROM {{ source('DEMO_FL', 'SAT_JDE_SALESORDERHEADER') }} "SAT_SRC"
		INNER JOIN "DIST_STG" "DIST_STG" ON  "SAT_SRC"."SALESORDERHEADER_HKEY" = "DIST_STG"."SALESORDERHEADER_HKEY"
	...
	SELECT 
		...
		, CASE WHEN "TEMP_TABLE_SET"."SOURCE" = 'STG' AND TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."HASH_DIFF")
			= LAG( TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."HASH_DIFF"),1)OVER(PARTITION BY "TEMP_TABLE_SET"."SALESORDERHEADER_HKEY" ORDER BY "TEMP_TABLE_SET"."LOAD_DATE","TEMP_TABLE_SET"."ORIGIN_ID")THEN 1 ELSE 0 END AS "EQUAL"

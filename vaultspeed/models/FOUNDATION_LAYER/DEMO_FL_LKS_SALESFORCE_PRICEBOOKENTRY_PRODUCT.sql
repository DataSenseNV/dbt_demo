{{
	config(
		...
		alias='LKS_SALESFORCE_PRICEBOOKENTRY_PRODUCT',
		schema='DEMO_FL',
		unique_key = ['"LNK_PRICEBOOKENTRY_PRODUCT_HKEY"', '"LOAD_DATE"'],
		merge_update_columns = ['"LOAD_END_DATE"', '"DELETE_FLAG"', '"CDC_TIMESTAMP"'],
		tags=['SALESFORCE', 'LKS_SALESFORCE_PRICEBOOKENTRY_PRODUCT_INCR', 'LKS_SALESFORCE_PRICEBOOKENTRY_PRODUCT_INIT']
	)
}}
select * from (
	WITH "END_DATING" AS 
	( 
		...
		FROM {{ ref('SALESFORCE_STG_LKS_SALESFORCE_PRICEBOOKENTRY_PRODUCT_TMP') }} "LKS_TEMP_SRC_DK"
		WHERE  "LKS_TEMP_SRC_DK"."EQUAL" = 0
	)
	, "CALC_LOAD_END_DATE" AS 
	( 
		SELECT 
			  "LKS_TEMP_SRC_US"."LNK_PRICEBOOKENTRY_PRODUCT_HKEY" AS "LNK_PRICEBOOKENTRY_PRODUCT_HKEY"
			...
			, "LKS_TEMP_SRC_US"."PRODUCT2ID" AS "PRODUCT2ID"
		FROM {{ ref('SALESFORCE_STG_LKS_SALESFORCE_PRICEBOOKENTRY_PRODUCT_TMP') }} "LKS_TEMP_SRC_US"
	...
	( 
		SELECT 
			  "CALC_LOAD_END_DATE"."LNK_PRICEBOOKENTRY_PRODUCT_HKEY" AS "LNK_PRICEBOOKENTRY_PRODUCT_HKEY"
	...
	SELECT 
		  "FILTER_LOAD_END_DATE"."LNK_PRICEBOOKENTRY_PRODUCT_HKEY"
...
select * from (
	...
	( 
		SELECT 
			  "STG_INR_SRC"."LNK_PRICEBOOKENTRY_PRODUCT_HKEY" AS "LNK_PRICEBOOKENTRY_PRODUCT_HKEY"
	...
	SELECT 
		  "STG_SRC"."LNK_PRICEBOOKENTRY_PRODUCT_HKEY" AS "LNK_PRICEBOOKENTRY_PRODUCT_HKEY"

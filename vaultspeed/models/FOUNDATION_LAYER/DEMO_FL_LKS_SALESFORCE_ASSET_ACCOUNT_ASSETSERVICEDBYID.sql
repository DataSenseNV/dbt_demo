{{
	config(
		...
		alias='LKS_SALESFORCE_ASSET_ACCOUNT_ASSETSERVICEDBYID',
		schema='DEMO_FL',
		unique_key = ['"LNK_ASSET_ACCOUNT_ASSETSERVICEDBYID_HKEY"', '"LOAD_DATE"'],
		merge_update_columns = ['"LOAD_END_DATE"', '"DELETE_FLAG"', '"CDC_TIMESTAMP"'],
		tags=['SALESFORCE', 'LKS_SALESFORCE_ASSET_ACCOUNT_ASSETSERVICEDBYID_INCR', 'LKS_SALESFORCE_ASSET_ACCOUNT_ASSETSERVICEDBYID_INIT']
	)
}}
select * from (
	WITH "END_DATING" AS 
	( 
		SELECT 
			  "LKS_TEMP_SRC_DK"."ASSET_HKEY" AS "ASSET_HKEY"
			, "LKS_TEMP_SRC_DK"."LOAD_DATE" AS "LOAD_DATE"
			, CASE WHEN LEAD( "LKS_TEMP_SRC_DK"."DELETE_FLAG" || "LKS_TEMP_SRC_DK"."SOURCE",1)OVER(PARTITION BY "LKS_TEMP_SRC_DK"."ASSET_HKEY" ORDER BY "LKS_TEMP_SRC_DK"."LOAD_DATE",
				"LKS_TEMP_SRC_DK"."LOAD_CYCLE_ID")= 'Y' || 'STG' THEN CAST('Y' AS VARCHAR) ELSE "LKS_TEMP_SRC_DK"."DELETE_FLAG" END AS "DELETE_FLAG"
			, LEAD("LKS_TEMP_SRC_DK"."CDC_TIMESTAMP",1)OVER(PARTITION BY "LKS_TEMP_SRC_DK"."ASSET_HKEY" ORDER BY "LKS_TEMP_SRC_DK"."LOAD_DATE",
				"LKS_TEMP_SRC_DK"."LOAD_CYCLE_ID") AS "CDC_TIMESTAMP"
		FROM {{ ref('SALESFORCE_STG_LKS_SALESFORCE_ASSET_ACCOUNT_ASSETSERVICEDBYID_TMP') }} "LKS_TEMP_SRC_DK"
		WHERE  "LKS_TEMP_SRC_DK"."EQUAL" = 0
	)
	, "CALC_LOAD_END_DATE" AS 
	( 
		SELECT 
			  "LKS_TEMP_SRC_US"."LNK_ASSET_ACCOUNT_ASSETSERVICEDBYID_HKEY" AS "LNK_ASSET_ACCOUNT_ASSETSERVICEDBYID_HKEY"
			, "LKS_TEMP_SRC_US"."LOAD_DATE" AS "LOAD_DATE"
			, "LKS_TEMP_SRC_US"."LOAD_CYCLE_ID" AS "LOAD_CYCLE_ID"
			, COALESCE(LEAD("LKS_TEMP_SRC_US"."LOAD_DATE",1)OVER(PARTITION BY "LKS_TEMP_SRC_US"."ASSET_HKEY" ORDER BY "LKS_TEMP_SRC_US"."LOAD_DATE",
			...
			, "LKS_TEMP_SRC_US"."ASSETSERVICEDBYID" AS "ASSETSERVICEDBYID"
		FROM {{ ref('SALESFORCE_STG_LKS_SALESFORCE_ASSET_ACCOUNT_ASSETSERVICEDBYID_TMP') }} "LKS_TEMP_SRC_US"
		INNER JOIN "END_DATING" "END_DATING" ON  "LKS_TEMP_SRC_US"."ASSET_HKEY" = "END_DATING"."ASSET_HKEY" AND "LKS_TEMP_SRC_US"."LOAD_DATE" = "END_DATING"."LOAD_DATE"
	...
	( 
		SELECT 
			  "CALC_LOAD_END_DATE"."LNK_ASSET_ACCOUNT_ASSETSERVICEDBYID_HKEY" AS "LNK_ASSET_ACCOUNT_ASSETSERVICEDBYID_HKEY"
	...
	SELECT 
		  "FILTER_LOAD_END_DATE"."LNK_ASSET_ACCOUNT_ASSETSERVICEDBYID_HKEY"
...
select * from (
	...
	( 
		SELECT 
			  "STG_INR_SRC"."LNK_ASSET_ACCOUNT_ASSETSERVICEDBYID_HKEY" AS "LNK_ASSET_ACCOUNT_ASSETSERVICEDBYID_HKEY"
	...
	SELECT 
		  "STG_SRC"."LNK_ASSET_ACCOUNT_ASSETSERVICEDBYID_HKEY" AS "LNK_ASSET_ACCOUNT_ASSETSERVICEDBYID_HKEY"

{{
	config(
		...
		alias='LKS_SALESFORCE_CAMPAIGN_USER_CREATEDBYID_TMP',
		schema='SALESFORCE_STG',
		tags=['SALESFORCE', 'LKS_SALESFORCE_CAMPAIGN_USER_CREATEDBYID_INCR']
	)
}}
select * from (
	WITH "DIST_STG" AS 
	...
	( 
		SELECT 
			  "STG_TEMP_SRC"."LNK_CAMPAIGN_USER_CREATEDBYID_HKEY" AS "LNK_CAMPAIGN_USER_CREATEDBYID_HKEY"
		...
		UNION 
		SELECT 
			  "LKS_SRC"."LNK_CAMPAIGN_USER_CREATEDBYID_HKEY" AS "LNK_CAMPAIGN_USER_CREATEDBYID_HKEY"
		...
		FROM {{ source('DEMO_FL', 'LKS_SALESFORCE_CAMPAIGN_USER_CREATEDBYID') }} "LKS_SRC"
		INNER JOIN {{ source('DEMO_FL', 'LNK_CAMPAIGN_USER_CREATEDBYID') }} "LNK_SRC" ON  "LKS_SRC"."LNK_CAMPAIGN_USER_CREATEDBYID_HKEY" = "LNK_SRC"."LNK_CAMPAIGN_USER_CREATEDBYID_HKEY"
	...
	SELECT 
		  "TEMP_TABLE_SET"."LNK_CAMPAIGN_USER_CREATEDBYID_HKEY" AS "LNK_CAMPAIGN_USER_CREATEDBYID_HKEY"
		...
		, CASE WHEN "TEMP_TABLE_SET"."SOURCE" = 'STG' AND TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."USER_CREATEDBYID_HKEY")
			= LAG( TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."USER_CREATEDBYID_HKEY"),1)OVER(PARTITION BY "TEMP_TABLE_SET"."CAMPAIGN_HKEY" ORDER BY "TEMP_TABLE_SET"."LOAD_DATE","TEMP_TABLE_SET"."ORIGIN_ID")THEN 1 ELSE 0 END AS "EQUAL"

{{
	config(
		...
		alias='LKS_SALESFORCE_ACCOUNT_USER_CREATEDBYID',
		schema='DEMO_FL',
		unique_key = ['"LNK_ACCOUNT_USER_CREATEDBYID_HKEY"', '"LOAD_DATE"'],
		merge_update_columns = ['"LOAD_END_DATE"', '"DELETE_FLAG"', '"CDC_TIMESTAMP"'],
		tags=['SALESFORCE', 'LKS_SALESFORCE_ACCOUNT_USER_CREATEDBYID_INCR', 'LKS_SALESFORCE_ACCOUNT_USER_CREATEDBYID_INIT']
	)
}}
select * from (
	WITH "END_DATING" AS 
	( 
		...
		FROM {{ ref('SALESFORCE_STG_LKS_SALESFORCE_ACCOUNT_USER_CREATEDBYID_TMP') }} "LKS_TEMP_SRC_DK"
		WHERE  "LKS_TEMP_SRC_DK"."EQUAL" = 0
	)
	, "CALC_LOAD_END_DATE" AS 
	( 
		SELECT 
			  "LKS_TEMP_SRC_US"."LNK_ACCOUNT_USER_CREATEDBYID_HKEY" AS "LNK_ACCOUNT_USER_CREATEDBYID_HKEY"
		...
		FROM {{ ref('SALESFORCE_STG_LKS_SALESFORCE_ACCOUNT_USER_CREATEDBYID_TMP') }} "LKS_TEMP_SRC_US"
	...
	( 
		SELECT 
			  "CALC_LOAD_END_DATE"."LNK_ACCOUNT_USER_CREATEDBYID_HKEY" AS "LNK_ACCOUNT_USER_CREATEDBYID_HKEY"
	...
	SELECT 
		  "FILTER_LOAD_END_DATE"."LNK_ACCOUNT_USER_CREATEDBYID_HKEY"
...
select * from (
	...
	( 
		SELECT 
			  "STG_INR_SRC"."LNK_ACCOUNT_USER_CREATEDBYID_HKEY" AS "LNK_ACCOUNT_USER_CREATEDBYID_HKEY"
	...
	SELECT 
		  "STG_SRC"."LNK_ACCOUNT_USER_CREATEDBYID_HKEY" AS "LNK_ACCOUNT_USER_CREATEDBYID_HKEY"

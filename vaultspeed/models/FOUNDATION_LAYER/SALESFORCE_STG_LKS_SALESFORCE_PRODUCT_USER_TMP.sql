{{
	config(
		...
		alias='LKS_SALESFORCE_PRODUCT_USER_TMP',
		schema='SALESFORCE_STG',
		tags=['SALESFORCE', 'LKS_SALESFORCE_PRODUCT_USER_INCR']
	)
}}
select * from (
	WITH "DIST_STG" AS 
	( 
		...
		GROUP BY  "STG_DIS_SRC"."PRODUCT_HKEY",  "STG_DIS_SRC"."LOAD_CYCLE_ID"
	)
	, "TEMP_TABLE_SET" AS 
	( 
		SELECT 
			  "STG_TEMP_SRC"."LNK_PRODUCT_USER_HKEY" AS "LNK_PRODUCT_USER_HKEY"
		...
		UNION 
		SELECT 
			  "LKS_SRC"."LNK_PRODUCT_USER_HKEY" AS "LNK_PRODUCT_USER_HKEY"
			, "LNK_SRC"."USER_HKEY" AS "USER_HKEY"
		...
		FROM {{ source('DEMO_FL', 'LKS_SALESFORCE_PRODUCT_USER') }} "LKS_SRC"
		INNER JOIN {{ source('DEMO_FL', 'LNK_PRODUCT_USER') }} "LNK_SRC" ON  "LKS_SRC"."LNK_PRODUCT_USER_HKEY" = "LNK_SRC"."LNK_PRODUCT_USER_HKEY"
		INNER JOIN "DIST_STG" "DIST_STG" ON  "LNK_SRC"."PRODUCT_HKEY" = "DIST_STG"."PRODUCT_HKEY"
		WHERE  "LKS_SRC"."LOAD_END_DATE" = TO_TIMESTAMP('31/12/2999 23:59:59' , 'DD/MM/YYYY HH24:MI:SS')
	)
	SELECT 
		  "TEMP_TABLE_SET"."LNK_PRODUCT_USER_HKEY" AS "LNK_PRODUCT_USER_HKEY"
		...
		, CASE WHEN "TEMP_TABLE_SET"."SOURCE" = 'STG' AND TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."USER_HKEY")
			= LAG( TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."USER_HKEY"),1)OVER(PARTITION BY "TEMP_TABLE_SET"."PRODUCT_HKEY" ORDER BY "TEMP_TABLE_SET"."LOAD_DATE","TEMP_TABLE_SET"."ORIGIN_ID")THEN 1 ELSE 0 END AS "EQUAL"

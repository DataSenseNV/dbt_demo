{{
	config(
		...
		alias='SAT_SALESFORCE_PARTNER',
		schema='DEMO_FL',
		unique_key = ['"PARTNER_HKEY"', '"LOAD_DATE"'],
		merge_update_columns = ['"LOAD_END_DATE"', '"DELETE_FLAG"', '"CDC_TIMESTAMP"'],
		tags=['SALESFORCE', 'SAT_SALESFORCE_PARTNER_INCR', 'SAT_SALESFORCE_PARTNER_INIT']
	)
}}
select * from (
	WITH "END_DATING" AS 
	( 
		SELECT 
			  "SAT_TEMP_SRC_DK"."PARTNER_HKEY" AS "PARTNER_HKEY"
			, "SAT_TEMP_SRC_DK"."LOAD_DATE" AS "LOAD_DATE"
			, CASE WHEN LEAD( "SAT_TEMP_SRC_DK"."DELETE_FLAG" || "SAT_TEMP_SRC_DK"."SOURCE",1)OVER(PARTITION BY "SAT_TEMP_SRC_DK"."PARTNER_HKEY" ORDER BY "SAT_TEMP_SRC_DK"."LOAD_DATE",
				"SAT_TEMP_SRC_DK"."LOAD_CYCLE_ID")= 'Y' || 'STG' THEN CAST('Y' AS VARCHAR) ELSE "SAT_TEMP_SRC_DK"."DELETE_FLAG" END AS "DELETE_FLAG"
			, LEAD("SAT_TEMP_SRC_DK"."CDC_TIMESTAMP",1)OVER(PARTITION BY "SAT_TEMP_SRC_DK"."PARTNER_HKEY" ORDER BY "SAT_TEMP_SRC_DK"."LOAD_DATE") AS "CDC_TIMESTAMP"
		FROM {{ ref('SALESFORCE_STG_SAT_SALESFORCE_PARTNER_TMP') }} "SAT_TEMP_SRC_DK"
		WHERE  "SAT_TEMP_SRC_DK"."EQUAL" = 0
	)
	, "CALC_LOAD_END_DATE" AS 
	( 
		SELECT 
			  "SAT_TEMP_SRC_US"."PARTNER_HKEY" AS "PARTNER_HKEY"
			, "SAT_TEMP_SRC_US"."LOAD_DATE" AS "LOAD_DATE"
			, "SAT_TEMP_SRC_US"."LOAD_CYCLE_ID" AS "LOAD_CYCLE_ID"
			, COALESCE(LEAD("SAT_TEMP_SRC_US"."LOAD_DATE",1)OVER(PARTITION BY "SAT_TEMP_SRC_US"."PARTNER_HKEY" ORDER BY "SAT_TEMP_SRC_US"."LOAD_DATE")
			...
			, "SAT_TEMP_SRC_US"."ACCOUNTTOID" AS "ACCOUNTTOID"
			, "SAT_TEMP_SRC_US"."REVERSEPARTNERID" AS "REVERSEPARTNERID"
			, "SAT_TEMP_SRC_US"."LASTMODIFIEDBYID" AS "LASTMODIFIEDBYID"
			, "SAT_TEMP_SRC_US"."OPPORTUNITYID" AS "OPPORTUNITYID"
			, "SAT_TEMP_SRC_US"."ACCOUNTFROMID" AS "ACCOUNTFROMID"
		...
		FROM {{ ref('SALESFORCE_STG_SAT_SALESFORCE_PARTNER_TMP') }} "SAT_TEMP_SRC_US"
		INNER JOIN "END_DATING" "END_DATING" ON  "SAT_TEMP_SRC_US"."PARTNER_HKEY" = "END_DATING"."PARTNER_HKEY" AND "SAT_TEMP_SRC_US"."LOAD_DATE" = 
	...
	( 
		SELECT 
			  "CALC_LOAD_END_DATE"."PARTNER_HKEY" AS "PARTNER_HKEY"
			...
			, "CALC_LOAD_END_DATE"."ACCOUNTTOID" AS "ACCOUNTTOID"
			, "CALC_LOAD_END_DATE"."REVERSEPARTNERID" AS "REVERSEPARTNERID"
	...
	SELECT 
		  "FILTER_LOAD_END_DATE"."PARTNER_HKEY"
		...
		, "FILTER_LOAD_END_DATE"."ACCOUNTTOID"
		, "FILTER_LOAD_END_DATE"."REVERSEPARTNERID"
...
select * from (
	...
	( 
		SELECT 
			  "STG_INR_SRC"."PARTNER_HKEY" AS "PARTNER_HKEY"
			...
			, "STG_INR_SRC"."ACCOUNTTOID" AS "ACCOUNTTOID"
			, "STG_INR_SRC"."REVERSEPARTNERID" AS "REVERSEPARTNERID"
	...
	SELECT 
		  "STG_SRC"."PARTNER_HKEY" AS "PARTNER_HKEY"
		...
		, "STG_SRC"."ID" AS "ID"
		, "STG_SRC"."ACCOUNTTOID" AS "ACCOUNTTOID"
		, "STG_SRC"."REVERSEPARTNERID" AS "REVERSEPARTNERID"

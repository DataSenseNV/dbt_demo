{{
	config(
		...
		alias='LKS_SALESFORCE_CONTACT_ACCOUNT_TMP',
		schema='SALESFORCE_STG',
		tags=['SALESFORCE', 'LKS_SALESFORCE_CONTACT_ACCOUNT_INCR']
	)
}}
select * from (
	WITH "DIST_STG" AS 
	...
	( 
		SELECT 
			  "STG_TEMP_SRC"."LNK_CONTACT_ACCOUNT_HKEY" AS "LNK_CONTACT_ACCOUNT_HKEY"
		...
		UNION 
		SELECT 
			  "LKS_SRC"."LNK_CONTACT_ACCOUNT_HKEY" AS "LNK_CONTACT_ACCOUNT_HKEY"
		...
		FROM {{ source('DEMO_FL', 'LKS_SALESFORCE_CONTACT_ACCOUNT') }} "LKS_SRC"
		INNER JOIN {{ source('DEMO_FL', 'LNK_CONTACT_ACCOUNT') }} "LNK_SRC" ON  "LKS_SRC"."LNK_CONTACT_ACCOUNT_HKEY" = "LNK_SRC"."LNK_CONTACT_ACCOUNT_HKEY"
	...
	SELECT 
		  "TEMP_TABLE_SET"."LNK_CONTACT_ACCOUNT_HKEY" AS "LNK_CONTACT_ACCOUNT_HKEY"
		...
		, CASE WHEN "TEMP_TABLE_SET"."SOURCE" = 'STG' AND TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."ACCOUNT_HKEY")
			= LAG( TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."ACCOUNT_HKEY"),1)OVER(PARTITION BY "TEMP_TABLE_SET"."CONTACT_HKEY" ORDER BY "TEMP_TABLE_SET"."LOAD_DATE","TEMP_TABLE_SET"."ORIGIN_ID")THEN 1 ELSE 0 END AS "EQUAL"

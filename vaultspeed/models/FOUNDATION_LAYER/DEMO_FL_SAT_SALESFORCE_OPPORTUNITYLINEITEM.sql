{{
	config(
		...
		schema='DEMO_FL',
		unique_key = ['"OPPORTUNITYLINEITEM_HKEY"', '"LOAD_DATE"'],
		merge_update_columns = ['"LOAD_END_DATE"', '"DELETE_FLAG"', '"CDC_TIMESTAMP"'],
		tags=['SALESFORCE', 'SAT_SALESFORCE_OPPORTUNITYLINEITEM_INCR', 'SAT_SALESFORCE_OPPORTUNITYLINEITEM_INIT']
	)
}}
select * from (
	WITH "END_DATING" AS 
	( 
		SELECT 
			  "SAT_TEMP_SRC_DK"."OPPORTUNITYLINEITEM_HKEY" AS "OPPORTUNITYLINEITEM_HKEY"
			, "SAT_TEMP_SRC_DK"."LOAD_DATE" AS "LOAD_DATE"
			, CASE WHEN LEAD( "SAT_TEMP_SRC_DK"."DELETE_FLAG" || "SAT_TEMP_SRC_DK"."SOURCE",1)OVER(PARTITION BY "SAT_TEMP_SRC_DK"."OPPORTUNITYLINEITEM_HKEY" ORDER BY "SAT_TEMP_SRC_DK"."LOAD_DATE",
				"SAT_TEMP_SRC_DK"."LOAD_CYCLE_ID")= 'Y' || 'STG' THEN CAST('Y' AS VARCHAR) ELSE "SAT_TEMP_SRC_DK"."DELETE_FLAG" END AS "DELETE_FLAG"
			, LEAD("SAT_TEMP_SRC_DK"."CDC_TIMESTAMP",1)OVER(PARTITION BY "SAT_TEMP_SRC_DK"."OPPORTUNITYLINEITEM_HKEY" ORDER BY "SAT_TEMP_SRC_DK"."LOAD_DATE") AS "CDC_TIMESTAMP"
		FROM {{ ref('SALESFORCE_STG_SAT_SALESFORCE_OPPORTUNITYLINEITEM_TMP') }} "SAT_TEMP_SRC_DK"
		WHERE  "SAT_TEMP_SRC_DK"."EQUAL" = 0
	)
	, "CALC_LOAD_END_DATE" AS 
	( 
		SELECT 
			  "SAT_TEMP_SRC_US"."OPPORTUNITYLINEITEM_HKEY" AS "OPPORTUNITYLINEITEM_HKEY"
			, "SAT_TEMP_SRC_US"."LOAD_DATE" AS "LOAD_DATE"
			, "SAT_TEMP_SRC_US"."LOAD_CYCLE_ID" AS "LOAD_CYCLE_ID"
			, COALESCE(LEAD("SAT_TEMP_SRC_US"."LOAD_DATE",1)OVER(PARTITION BY "SAT_TEMP_SRC_US"."OPPORTUNITYLINEITEM_HKEY" ORDER BY "SAT_TEMP_SRC_US"."LOAD_DATE")
			...
			, "SAT_TEMP_SRC_US"."SORTORDER" AS "SORTORDER"
			, "SAT_TEMP_SRC_US"."QUANTITY" AS "QUANTITY"
			, "SAT_TEMP_SRC_US"."DISCOUNT" AS "DISCOUNT"
			, "SAT_TEMP_SRC_US"."TOTALPRICE" AS "TOTALPRICE"
		...
		FROM {{ ref('SALESFORCE_STG_SAT_SALESFORCE_OPPORTUNITYLINEITEM_TMP') }} "SAT_TEMP_SRC_US"
		INNER JOIN "END_DATING" "END_DATING" ON  "SAT_TEMP_SRC_US"."OPPORTUNITYLINEITEM_HKEY" = "END_DATING"."OPPORTUNITYLINEITEM_HKEY" AND "SAT_TEMP_SRC_US"."LOAD_DATE" =
	...
	( 
		SELECT 
			  "CALC_LOAD_END_DATE"."OPPORTUNITYLINEITEM_HKEY" AS "OPPORTUNITYLINEITEM_HKEY"
			...
			, "CALC_LOAD_END_DATE"."SORTORDER" AS "SORTORDER"
			, "CALC_LOAD_END_DATE"."QUANTITY" AS "QUANTITY"
			, "CALC_LOAD_END_DATE"."DISCOUNT" AS "DISCOUNT"
			, "CALC_LOAD_END_DATE"."TOTALPRICE" AS "TOTALPRICE"
	...
	SELECT 
		  "FILTER_LOAD_END_DATE"."OPPORTUNITYLINEITEM_HKEY"
		...
		, "FILTER_LOAD_END_DATE"."SORTORDER"
		, "FILTER_LOAD_END_DATE"."QUANTITY"
		, "FILTER_LOAD_END_DATE"."DISCOUNT"
		, "FILTER_LOAD_END_DATE"."TOTALPRICE"
...
select * from (
	...
	( 
		SELECT 
			  "STG_INR_SRC"."OPPORTUNITYLINEITEM_HKEY" AS "OPPORTUNITYLINEITEM_HKEY"
			, "STG_INR_SRC"."LOAD_DATE" AS "LOAD_DATE"
			, TO_TIMESTAMP('31/12/2999 23:59:59' , 'DD/MM/YYYY HH24:MI:SS') AS "LOAD_END_DATE"
			, "STG_INR_SRC"."LOAD_CYCLE_ID" AS "LOAD_CYCLE_ID"
			, UPPER(MD5_HEX(COALESCE(RTRIM( UPPER(REPLACE(COALESCE(TRIM( "STG_INR_SRC"."SORTORDER"),'~'),'\#','\\' || '\#')
				)|| '\#' ||  UPPER(REPLACE(COALESCE(TRIM( TO_CHAR("STG_INR_SRC"."QUANTITY")),'~'),'\#','\\' || '\#'))|| '\#' ||  UPPER(REPLACE(COALESCE(TRIM( "STG_INR_SRC"."DISCOUNT"),'~'),'\#','\\' || '\#'))|| '\#' ||  UPPER(REPLACE(COALESCE(TRIM( TO_CHAR("STG_INR_SRC"."TOTALPRICE")),'~'),'\#','\\' || '\#'))|| '\#' ||  UPPER(REPLACE(COALESCE(TRIM( TO_CHAR("STG_INR_SRC"."UNITPRICE")),'~'),'\#','\\' || '\#'))|| '\#' ||  UPPER(REPLACE(COALESCE(TRIM( "STG_INR_SRC"."SERVICEDATE"),'~'),'\#','\\' || '\#'))|| '\#' ||  UPPER(REPLACE(COALESCE(TRIM( "STG_INR_SRC"."DESCRIPTION"),'~'),'\#','\\' || '\#'))|| '\#' ||  UPPER(REPLACE(COALESCE(TRIM( "STG_INR_SRC"."CREATEDDATE"),'~'),'\#','\\' || '\#'))|| '\#' ||  UPPER(REPLACE(COALESCE(TRIM( "STG_INR_SRC"."LASTMODIFIEDDATE"),'~'),'\#','\\' || '\#'))|| '\#' ||  UPPER(REPLACE(COALESCE(TRIM( "STG_INR_SRC"."SYSTEMMODSTAMP"),'~'),'\#','\\' || '\#'))|| '\#' ||  UPPER(REPLACE(COALESCE(TRIM( TO_CHAR("STG_INR_SRC"."ISDELETED")),'~'),'\#','\\' || '\#'))|| '\#','\#' || '~'),'~') )) AS "HASH_DIFF"
			...
			, "STG_INR_SRC"."SORTORDER" AS "SORTORDER"
			, "STG_INR_SRC"."QUANTITY" AS "QUANTITY"
			, "STG_INR_SRC"."DISCOUNT" AS "DISCOUNT"
			, "STG_INR_SRC"."TOTALPRICE" AS "TOTALPRICE"
			...
			, ROW_NUMBER()OVER(PARTITION BY "STG_INR_SRC"."OPPORTUNITYLINEITEM_HKEY" ORDER BY "STG_INR_SRC"."LOAD_DATE") AS "DUMMY"
		FROM {{ ref('SALESFORCE_STG_OPPORTUNITYLINEITEM') }} "STG_INR_SRC"
	)
	SELECT 
		  "STG_SRC"."OPPORTUNITYLINEITEM_HKEY" AS "OPPORTUNITYLINEITEM_HKEY"
		...
		, "STG_SRC"."SORTORDER" AS "SORTORDER"
		, "STG_SRC"."QUANTITY" AS "QUANTITY"
		, "STG_SRC"."DISCOUNT" AS "DISCOUNT"
		, "STG_SRC"."TOTALPRICE" AS "TOTALPRICE"

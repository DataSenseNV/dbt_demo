{{
	config(
		...
		alias='SAT_SALESFORCE_CONTRACTCONTACTROLE_TMP',
		schema='SALESFORCE_STG',
		tags=['SALESFORCE', 'SAT_SALESFORCE_CONTRACTCONTACTROLE_INCR']
	)
}}
select * from (
	WITH "DIST_STG" AS 
	...
	( 
		...
		UNION ALL 
		SELECT 
			  "SAT_SRC"."CONTRACTCONTACTROLE_HKEY" AS "CONTRACTCONTACTROLE_HKEY"
		...
		FROM {{ source('DEMO_FL', 'SAT_SALESFORCE_CONTRACTCONTACTROLE') }} "SAT_SRC"
		INNER JOIN "DIST_STG" "DIST_STG" ON  "SAT_SRC"."CONTRACTCONTACTROLE_HKEY" = "DIST_STG"."CONTRACTCONTACTROLE_HKEY"
	...
	SELECT 
		...
		, CASE WHEN "TEMP_TABLE_SET"."SOURCE" = 'STG' AND TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."HASH_DIFF")
			= LAG( TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."HASH_DIFF"),1)OVER(PARTITION BY "TEMP_TABLE_SET"."CONTRACTCONTACTROLE_HKEY" ORDER BY "TEMP_TABLE_SET"."LOAD_DATE","TEMP_TABLE_SET"."ORIGIN_ID")THEN 1 ELSE 0 END AS "EQUAL"

{{
	config(
		...
		alias='LKS_JDE_EMAIL_WHOISWHO_TMP',
		schema='JDEDWARDS_STG',
		tags=['JDEDWARDS', 'LKS_JDE_EMAIL_WHOISWHO_INCR']
	)
}}
select * from (
	WITH "DIST_STG" AS 
	...
	( 
		SELECT 
			  "STG_TEMP_SRC"."LNK_EMAIL_WHOISWHO_HKEY" AS "LNK_EMAIL_WHOISWHO_HKEY"
		...
		UNION 
		SELECT 
			  "LKS_SRC"."LNK_EMAIL_WHOISWHO_HKEY" AS "LNK_EMAIL_WHOISWHO_HKEY"
		...
		FROM {{ source('DEMO_FL', 'LKS_JDE_EMAIL_WHOISWHO') }} "LKS_SRC"
		INNER JOIN {{ source('DEMO_FL', 'LNK_EMAIL_WHOISWHO') }} "LNK_SRC" ON  "LKS_SRC"."LNK_EMAIL_WHOISWHO_HKEY" = "LNK_SRC"."LNK_EMAIL_WHOISWHO_HKEY"
	...
	SELECT 
		  "TEMP_TABLE_SET"."LNK_EMAIL_WHOISWHO_HKEY" AS "LNK_EMAIL_WHOISWHO_HKEY"
		...
		, CASE WHEN "TEMP_TABLE_SET"."SOURCE" = 'STG' AND TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."WHOISWHO_HKEY")
			= LAG( TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."WHOISWHO_HKEY"),1)OVER(PARTITION BY "TEMP_TABLE_SET"."EMAIL_HKEY" ORDER BY "TEMP_TABLE_SET"."LOAD_DATE","TEMP_TABLE_SET"."ORIGIN_ID")THEN 1 ELSE 0 END AS "EQUAL"

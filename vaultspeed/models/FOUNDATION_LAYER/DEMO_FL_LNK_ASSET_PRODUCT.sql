{{
	config(
		...
		alias='LNK_ASSET_PRODUCT',
		schema='DEMO_FL',
		unique_key = ['"LNK_ASSET_PRODUCT_HKEY"'],
		merge_update_columns = [],
		tags=['SALESFORCE', 'LNK_SALESFORCE_ASSET_PRODUCT_INCR', 'LNK_SALESFORCE_ASSET_PRODUCT_INIT']
	)
}}
select * from (
	WITH "CHANGE_SET" AS 
	( 
		SELECT 
			  "STG_SRC1"."LNK_ASSET_PRODUCT_HKEY" AS "LNK_ASSET_PRODUCT_HKEY"
	...
	( 
		SELECT 
			  "CHANGE_SET"."LNK_ASSET_PRODUCT_HKEY" AS "LNK_ASSET_PRODUCT_HKEY"
			, "CHANGE_SET"."LOAD_DATE" AS "LOAD_DATE"
			, "CHANGE_SET"."LOAD_CYCLE_ID" AS "LOAD_CYCLE_ID"
			, "CHANGE_SET"."PRODUCT_HKEY" AS "PRODUCT_HKEY"
			, "CHANGE_SET"."ASSET_HKEY" AS "ASSET_HKEY"
			, ROW_NUMBER()OVER(PARTITION BY "CHANGE_SET"."LNK_ASSET_PRODUCT_HKEY" ORDER BY "CHANGE_SET"."LOAD_DATE",
	...
	( 
		SELECT 
			  "MIN_LOAD_TIME"."LNK_ASSET_PRODUCT_HKEY" AS "LNK_ASSET_PRODUCT_HKEY"
...
select * from (
	...
	( 
		SELECT 
			...
			, "CHANGE_SET"."PRODUCT_HKEY" AS "PRODUCT_HKEY"
			, "CHANGE_SET"."ASSET_HKEY" AS "ASSET_HKEY"
			, ROW_NUMBER()OVER(PARTITION BY "CHANGE_SET"."LNK_ASSET_PRODUCT_HKEY" ORDER BY "CHANGE_SET"."LOAD_CYCLE_ID",

{{
	config(
		...
		schema='DEMO_FL',
		unique_key = ['"LNK_SODETAILHIST_SOHEADER_HKEY"'],
		merge_update_columns = [],
		tags=['JDEDWARDS', 'LNK_JDE_SODETAILHIST_SOHEADER_INCR', 'LNK_JDE_SODETAILHIST_SOHEADER_INIT']
	)
}}
select * from (
	WITH "CHANGE_SET" AS 
	( 
		SELECT 
			  "STG_SRC1"."LNK_SODETAILHIST_SOHEADER_HKEY" AS "LNK_SODETAILHIST_SOHEADER_HKEY"
	...
	( 
		SELECT 
			  "CHANGE_SET"."LNK_SODETAILHIST_SOHEADER_HKEY" AS "LNK_SODETAILHIST_SOHEADER_HKEY"
			...
			, ROW_NUMBER()OVER(PARTITION BY "CHANGE_SET"."LNK_SODETAILHIST_SOHEADER_HKEY" ORDER BY "CHANGE_SET"."LOAD_DATE",
	...
	( 
		SELECT 
			  "MIN_LOAD_TIME"."LNK_SODETAILHIST_SOHEADER_HKEY" AS "LNK_SODETAILHIST_SOHEADER_HKEY"
	...
	SELECT 
		  "MIV"."LNK_SODETAILHIST_SOHEADER_HKEY"
		, "MIV"."LOAD_DATE"
		, "MIV"."LOAD_CYCLE_ID"
...
select * from (
	...
	( 
		SELECT 
			...
			, "CHANGE_SET"."SALESORDERHEADER_HKEY" AS "SALESORDERHEADER_HKEY"
			, "CHANGE_SET"."SALESORDERDETAILHISTORY_HKEY" AS "SALESORDERDETAILHISTORY_HKEY"
			, ROW_NUMBER()OVER(PARTITION BY "CHANGE_SET"."LNK_SODETAILHIST_SOHEADER_HKEY" ORDER BY "CHANGE_SET"."LOAD_CYCLE_ID",

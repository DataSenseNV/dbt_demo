{{
	config(
		...
		alias='LKS_JDE_BUSINESSUNITMASTER_COMPANY_TMP',
		schema='JDEDWARDS_STG',
		tags=['JDEDWARDS', 'LKS_JDE_BUSINESSUNITMASTER_COMPANY_INCR']
	)
}}
select * from (
	WITH "DIST_STG" AS 
	...
	( 
		SELECT 
			  "STG_TEMP_SRC"."LNK_BUSINESSUNITMASTER_COMPANY_HKEY" AS "LNK_BUSINESSUNITMASTER_COMPANY_HKEY"
		...
		UNION 
		SELECT 
			  "LKS_SRC"."LNK_BUSINESSUNITMASTER_COMPANY_HKEY" AS "LNK_BUSINESSUNITMASTER_COMPANY_HKEY"
		...
		FROM {{ source('DEMO_FL', 'LKS_JDE_BUSINESSUNITMASTER_COMPANY') }} "LKS_SRC"
		INNER JOIN {{ source('DEMO_FL', 'LNK_BUSINESSUNITMASTER_COMPANY') }} "LNK_SRC" ON  "LKS_SRC"."LNK_BUSINESSUNITMASTER_COMPANY_HKEY" = "LNK_SRC"."LNK_BUSINESSUNITMASTER_COMPANY_HKEY"
	...
	SELECT 
		  "TEMP_TABLE_SET"."LNK_BUSINESSUNITMASTER_COMPANY_HKEY" AS "LNK_BUSINESSUNITMASTER_COMPANY_HKEY"
		...
		, CASE WHEN "TEMP_TABLE_SET"."SOURCE" = 'STG' AND TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."COMPANY_HKEY")
			= LAG( TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."COMPANY_HKEY"),1)OVER(PARTITION BY "TEMP_TABLE_SET"."BUSINESSUNITMASTER_HKEY" ORDER BY "TEMP_TABLE_SET"."LOAD_DATE","TEMP_TABLE_SET"."ORIGIN_ID")THEN 1 ELSE 0 END AS "EQUAL"

{{
	config(
		...
		schema='DEMO_FL',
		unique_key = ['"LNK_ORDERS_ORDERS_ORIGINALORDERID_HKEY"', '"LOAD_DATE"'],
		merge_update_columns = ['"LOAD_END_DATE"', '"DELETE_FLAG"', '"CDC_TIMESTAMP"'],
		tags=['SALESFORCE', 'LKS_SALESFORCE_ORDERS_ORDERS_ORIGINALORDERID_INCR', 'LKS_SALESFORCE_ORDERS_ORDERS_ORIGINALORDERID_INIT']
	)
}}
select * from (
	WITH "END_DATING" AS 
	( 
		...
		FROM {{ ref('SALESFORCE_STG_LKS_SALESFORCE_ORDERS_ORDERS_ORIGINALORDERID_TMP') }} "LKS_TEMP_SRC_DK"
		WHERE  "LKS_TEMP_SRC_DK"."EQUAL" = 0
	)
	, "CALC_LOAD_END_DATE" AS 
	( 
		SELECT 
			  "LKS_TEMP_SRC_US"."LNK_ORDERS_ORDERS_ORIGINALORDERID_HKEY" AS "LNK_ORDERS_ORDERS_ORIGINALORDERID_HKEY"
			...
			, "LKS_TEMP_SRC_US"."ORIGINALORDERID" AS "ORIGINALORDERID"
		FROM {{ ref('SALESFORCE_STG_LKS_SALESFORCE_ORDERS_ORDERS_ORIGINALORDERID_TMP') }} "LKS_TEMP_SRC_US"
	...
	( 
		SELECT 
			  "CALC_LOAD_END_DATE"."LNK_ORDERS_ORDERS_ORIGINALORDERID_HKEY" AS "LNK_ORDERS_ORDERS_ORIGINALORDERID_HKEY"
	...
	SELECT 
		  "FILTER_LOAD_END_DATE"."LNK_ORDERS_ORDERS_ORIGINALORDERID_HKEY"
...
select * from (
	...
	( 
		SELECT 
			  "STG_INR_SRC"."LNK_ORDERS_ORDERS_ORIGINALORDERID_HKEY" AS "LNK_ORDERS_ORDERS_ORIGINALORDERID_HKEY"
	...
	SELECT 
		  "STG_SRC"."LNK_ORDERS_ORDERS_ORIGINALORDERID_HKEY" AS "LNK_ORDERS_ORDERS_ORIGINALORDERID_HKEY"

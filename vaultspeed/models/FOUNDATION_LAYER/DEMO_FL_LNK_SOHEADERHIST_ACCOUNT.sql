{{
	config(
		...
		schema='DEMO_FL',
		unique_key = ['"LNK_SOHEADERHIST_ACCOUNT_HKEY"'],
		merge_update_columns = [],
		tags=['JDEDWARDS', 'LNK_JDE_SOHEADERHIST_ACCOUNT_INCR', 'LNK_JDE_SOHEADERHIST_ACCOUNT_INIT']
	)
}}
select * from (
	WITH "CHANGE_SET" AS 
	( 
		SELECT 
			  "STG_SRC1"."LNK_SOHEADERHIST_ACCOUNT_HKEY" AS "LNK_SOHEADERHIST_ACCOUNT_HKEY"
	...
	( 
		SELECT 
			  "CHANGE_SET"."LNK_SOHEADERHIST_ACCOUNT_HKEY" AS "LNK_SOHEADERHIST_ACCOUNT_HKEY"
			...
			, ROW_NUMBER()OVER(PARTITION BY "CHANGE_SET"."LNK_SOHEADERHIST_ACCOUNT_HKEY" ORDER BY "CHANGE_SET"."LOAD_DATE",
	...
	( 
		SELECT 
			  "MIN_LOAD_TIME"."LNK_SOHEADERHIST_ACCOUNT_HKEY" AS "LNK_SOHEADERHIST_ACCOUNT_HKEY"
	...
	SELECT 
		  "MIV"."LNK_SOHEADERHIST_ACCOUNT_HKEY"
		, "MIV"."LOAD_DATE"
		, "MIV"."LOAD_CYCLE_ID"
		, "MIV"."ACCOUNT_HKEY"
...
select * from (
	...
	( 
		SELECT 
			...
			, "CHANGE_SET"."ACCOUNT_HKEY" AS "ACCOUNT_HKEY"
			, "CHANGE_SET"."SALESORDERHEADERHISTORY_HKEY" AS "SALESORDERHEADERHISTORY_HKEY"
			, ROW_NUMBER()OVER(PARTITION BY "CHANGE_SET"."LNK_SOHEADERHIST_ACCOUNT_HKEY" ORDER BY "CHANGE_SET"."LOAD_CYCLE_ID",

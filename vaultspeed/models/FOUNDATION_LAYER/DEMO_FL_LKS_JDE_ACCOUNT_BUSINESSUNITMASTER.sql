{{
	config(
		...
		schema='DEMO_FL',
		unique_key = ['"LNK_ACCOUNT_BUSINESSUNITMASTER_HKEY"', '"LOAD_DATE"'],
		merge_update_columns = ['"LOAD_END_DATE"', '"DELETE_FLAG"', '"CDC_TIMESTAMP"'],
		tags=['JDEDWARDS', 'LKS_JDE_ACCOUNT_BUSINESSUNITMASTER_INCR', 'LKS_JDE_ACCOUNT_BUSINESSUNITMASTER_INIT']
	)
}}
select * from (
	WITH "END_DATING" AS 
	( 
		...
		FROM {{ ref('JDEDWARDS_STG_LKS_JDE_ACCOUNT_BUSINESSUNITMASTER_TMP') }} "LKS_TEMP_SRC_DK"
		WHERE  "LKS_TEMP_SRC_DK"."EQUAL" = 0
	)
	, "CALC_LOAD_END_DATE" AS 
	( 
		SELECT 
			  "LKS_TEMP_SRC_US"."LNK_ACCOUNT_BUSINESSUNITMASTER_HKEY" AS "LNK_ACCOUNT_BUSINESSUNITMASTER_HKEY"
			...
			, "LKS_TEMP_SRC_US"."ABAN8" AS "ABAN8"
			, "LKS_TEMP_SRC_US"."ABMCU" AS "ABMCU"
		FROM {{ ref('JDEDWARDS_STG_LKS_JDE_ACCOUNT_BUSINESSUNITMASTER_TMP') }} "LKS_TEMP_SRC_US"
	...
	( 
		SELECT 
			  "CALC_LOAD_END_DATE"."LNK_ACCOUNT_BUSINESSUNITMASTER_HKEY" AS "LNK_ACCOUNT_BUSINESSUNITMASTER_HKEY"
	...
	SELECT 
		  "FILTER_LOAD_END_DATE"."LNK_ACCOUNT_BUSINESSUNITMASTER_HKEY"
...
select * from (
	...
	( 
		SELECT 
			  "STG_INR_SRC"."LNK_ACCOUNT_BUSINESSUNITMASTER_HKEY" AS "LNK_ACCOUNT_BUSINESSUNITMASTER_HKEY"
	...
	SELECT 
		  "STG_SRC"."LNK_ACCOUNT_BUSINESSUNITMASTER_HKEY" AS "LNK_ACCOUNT_BUSINESSUNITMASTER_HKEY"

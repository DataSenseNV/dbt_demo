{{
	config(
		...
		schema='DEMO_FL',
		unique_key = ['"LNK_EMAIL_ACCOUNT_HKEY"'],
		merge_update_columns = [],
		tags=['JDEDWARDS', 'LNK_JDE_EMAIL_ACCOUNT_INCR', 'LNK_JDE_EMAIL_ACCOUNT_INIT']
	)
}}
select * from (
	WITH "CHANGE_SET" AS 
	( 
		SELECT 
			  "STG_SRC1"."LNK_EMAIL_ACCOUNT_HKEY" AS "LNK_EMAIL_ACCOUNT_HKEY"
	...
	( 
		SELECT 
			  "CHANGE_SET"."LNK_EMAIL_ACCOUNT_HKEY" AS "LNK_EMAIL_ACCOUNT_HKEY"
			, "CHANGE_SET"."LOAD_DATE" AS "LOAD_DATE"
			, "CHANGE_SET"."LOAD_CYCLE_ID" AS "LOAD_CYCLE_ID"
			, "CHANGE_SET"."ACCOUNT_HKEY" AS "ACCOUNT_HKEY"
			, "CHANGE_SET"."EMAIL_HKEY" AS "EMAIL_HKEY"
			, ROW_NUMBER()OVER(PARTITION BY "CHANGE_SET"."LNK_EMAIL_ACCOUNT_HKEY" ORDER BY "CHANGE_SET"."LOAD_DATE",
	...
	( 
		SELECT 
			  "MIN_LOAD_TIME"."LNK_EMAIL_ACCOUNT_HKEY" AS "LNK_EMAIL_ACCOUNT_HKEY"
...
select * from (
	...
	( 
		SELECT 
			...
			, "CHANGE_SET"."ACCOUNT_HKEY" AS "ACCOUNT_HKEY"
			, "CHANGE_SET"."EMAIL_HKEY" AS "EMAIL_HKEY"
			, ROW_NUMBER()OVER(PARTITION BY "CHANGE_SET"."LNK_EMAIL_ACCOUNT_HKEY" ORDER BY "CHANGE_SET"."LOAD_CYCLE_ID",

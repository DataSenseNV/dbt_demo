{{
	config(
		...
		schema='DEMO_FL',
		unique_key = ['"LNK_COMPANY_ACCOUNT_HKEY"', '"LOAD_DATE"'],
		merge_update_columns = ['"LOAD_END_DATE"', '"DELETE_FLAG"', '"CDC_TIMESTAMP"'],
		tags=['JDEDWARDS', 'LKS_JDE_COMPANY_ACCOUNT_INCR', 'LKS_JDE_COMPANY_ACCOUNT_INIT']
	)
}}
select * from (
	WITH "END_DATING" AS 
	( 
		SELECT 
			  "LKS_TEMP_SRC_DK"."COMPANY_HKEY" AS "COMPANY_HKEY"
			, "LKS_TEMP_SRC_DK"."LOAD_DATE" AS "LOAD_DATE"
			, CASE WHEN LEAD( "LKS_TEMP_SRC_DK"."DELETE_FLAG" || "LKS_TEMP_SRC_DK"."SOURCE",1)OVER(PARTITION BY "LKS_TEMP_SRC_DK"."COMPANY_HKEY" ORDER BY "LKS_TEMP_SRC_DK"."LOAD_DATE",
				"LKS_TEMP_SRC_DK"."LOAD_CYCLE_ID")= 'Y' || 'STG' THEN CAST('Y' AS VARCHAR) ELSE "LKS_TEMP_SRC_DK"."DELETE_FLAG" END AS "DELETE_FLAG"
			, LEAD("LKS_TEMP_SRC_DK"."CDC_TIMESTAMP",1)OVER(PARTITION BY "LKS_TEMP_SRC_DK"."COMPANY_HKEY" ORDER BY "LKS_TEMP_SRC_DK"."LOAD_DATE",
				"LKS_TEMP_SRC_DK"."LOAD_CYCLE_ID") AS "CDC_TIMESTAMP"
		FROM {{ ref('JDEDWARDS_STG_LKS_JDE_COMPANY_ACCOUNT_TMP') }} "LKS_TEMP_SRC_DK"
		WHERE  "LKS_TEMP_SRC_DK"."EQUAL" = 0
	)
	, "CALC_LOAD_END_DATE" AS 
	( 
		SELECT 
			  "LKS_TEMP_SRC_US"."LNK_COMPANY_ACCOUNT_HKEY" AS "LNK_COMPANY_ACCOUNT_HKEY"
			, "LKS_TEMP_SRC_US"."LOAD_DATE" AS "LOAD_DATE"
			, "LKS_TEMP_SRC_US"."LOAD_CYCLE_ID" AS "LOAD_CYCLE_ID"
			, COALESCE(LEAD("LKS_TEMP_SRC_US"."LOAD_DATE",1)OVER(PARTITION BY "LKS_TEMP_SRC_US"."COMPANY_HKEY" ORDER BY "LKS_TEMP_SRC_US"."LOAD_DATE",
			...
			, "LKS_TEMP_SRC_US"."CCCO" AS "CCCO"
			, "LKS_TEMP_SRC_US"."CCAN8" AS "CCAN8"
		FROM {{ ref('JDEDWARDS_STG_LKS_JDE_COMPANY_ACCOUNT_TMP') }} "LKS_TEMP_SRC_US"
		INNER JOIN "END_DATING" "END_DATING" ON  "LKS_TEMP_SRC_US"."COMPANY_HKEY" = "END_DATING"."COMPANY_HKEY" AND "LKS_TEMP_SRC_US"."LOAD_DATE" = 
	...
	( 
		SELECT 
			  "CALC_LOAD_END_DATE"."LNK_COMPANY_ACCOUNT_HKEY" AS "LNK_COMPANY_ACCOUNT_HKEY"
	...
	SELECT 
		  "FILTER_LOAD_END_DATE"."LNK_COMPANY_ACCOUNT_HKEY"
...
select * from (
	...
	( 
		SELECT 
			  "STG_INR_SRC"."LNK_COMPANY_ACCOUNT_HKEY" AS "LNK_COMPANY_ACCOUNT_HKEY"
	...
	SELECT 
		  "STG_SRC"."LNK_COMPANY_ACCOUNT_HKEY" AS "LNK_COMPANY_ACCOUNT_HKEY"

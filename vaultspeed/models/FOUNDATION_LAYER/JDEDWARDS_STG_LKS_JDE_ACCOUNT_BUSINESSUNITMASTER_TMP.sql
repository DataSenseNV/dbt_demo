{{
	config(
		...
		alias='LKS_JDE_ACCOUNT_BUSINESSUNITMASTER_TMP',
		schema='JDEDWARDS_STG',
		tags=['JDEDWARDS', 'LKS_JDE_ACCOUNT_BUSINESSUNITMASTER_INCR']
	)
}}
select * from (
	WITH "DIST_STG" AS 
	...
	( 
		SELECT 
			  "STG_TEMP_SRC"."LNK_ACCOUNT_BUSINESSUNITMASTER_HKEY" AS "LNK_ACCOUNT_BUSINESSUNITMASTER_HKEY"
		...
		UNION 
		SELECT 
			  "LKS_SRC"."LNK_ACCOUNT_BUSINESSUNITMASTER_HKEY" AS "LNK_ACCOUNT_BUSINESSUNITMASTER_HKEY"
			...
			, "LKS_SRC"."ABAN8" AS "ABAN8"
			, "LKS_SRC"."ABMCU" AS "ABMCU"
		FROM {{ source('DEMO_FL', 'LKS_JDE_ACCOUNT_BUSINESSUNITMASTER') }} "LKS_SRC"
		INNER JOIN {{ source('DEMO_FL', 'LNK_ACCOUNT_BUSINESSUNITMASTER') }} "LNK_SRC" ON  "LKS_SRC"."LNK_ACCOUNT_BUSINESSUNITMASTER_HKEY" = "LNK_SRC"."LNK_ACCOUNT_BUSINESSUNITMASTER_HKEY"
	...
	SELECT 
		  "TEMP_TABLE_SET"."LNK_ACCOUNT_BUSINESSUNITMASTER_HKEY" AS "LNK_ACCOUNT_BUSINESSUNITMASTER_HKEY"
		...
		, CASE WHEN "TEMP_TABLE_SET"."SOURCE" = 'STG' AND TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."BUSINESSUNITMASTER_HKEY")
			= LAG( TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."BUSINESSUNITMASTER_HKEY"),1)OVER(PARTITION BY "TEMP_TABLE_SET"."ACCOUNT_HKEY" ORDER BY "TEMP_TABLE_SET"."LOAD_DATE","TEMP_TABLE_SET"."ORIGIN_ID")THEN 1 ELSE 0 END AS "EQUAL"

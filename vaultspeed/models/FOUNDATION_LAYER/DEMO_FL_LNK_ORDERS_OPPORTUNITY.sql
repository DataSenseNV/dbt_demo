{{
	config(
		...
		alias='LNK_ORDERS_OPPORTUNITY',
		schema='DEMO_FL',
		unique_key = ['"LNK_ORDERS_OPPORTUNITY_HKEY"'],
		merge_update_columns = [],
		tags=['SALESFORCE', 'LNK_SALESFORCE_ORDERS_OPPORTUNITY_INCR', 'LNK_SALESFORCE_ORDERS_OPPORTUNITY_INIT']
	)
}}
select * from (
	WITH "CHANGE_SET" AS 
	( 
		SELECT 
			  "STG_SRC1"."LNK_ORDERS_OPPORTUNITY_HKEY" AS "LNK_ORDERS_OPPORTUNITY_HKEY"
	...
	( 
		SELECT 
			  "CHANGE_SET"."LNK_ORDERS_OPPORTUNITY_HKEY" AS "LNK_ORDERS_OPPORTUNITY_HKEY"
			, "CHANGE_SET"."LOAD_DATE" AS "LOAD_DATE"
			, "CHANGE_SET"."LOAD_CYCLE_ID" AS "LOAD_CYCLE_ID"
			, "CHANGE_SET"."OPPORTUNITY_HKEY" AS "OPPORTUNITY_HKEY"
			, "CHANGE_SET"."ORDERS_HKEY" AS "ORDERS_HKEY"
			, ROW_NUMBER()OVER(PARTITION BY "CHANGE_SET"."LNK_ORDERS_OPPORTUNITY_HKEY" ORDER BY "CHANGE_SET"."LOAD_DATE",
	...
	( 
		SELECT 
			  "MIN_LOAD_TIME"."LNK_ORDERS_OPPORTUNITY_HKEY" AS "LNK_ORDERS_OPPORTUNITY_HKEY"
	...
	SELECT 
		  "MIV"."LNK_ORDERS_OPPORTUNITY_HKEY"
		, "MIV"."LOAD_DATE"
		, "MIV"."LOAD_CYCLE_ID"
...
select * from (
	...
	( 
		SELECT 
			...
			, "CHANGE_SET"."OPPORTUNITY_HKEY" AS "OPPORTUNITY_HKEY"
			, "CHANGE_SET"."ORDERS_HKEY" AS "ORDERS_HKEY"
			, ROW_NUMBER()OVER(PARTITION BY "CHANGE_SET"."LNK_ORDERS_OPPORTUNITY_HKEY" ORDER BY "CHANGE_SET"."LOAD_CYCLE_ID",

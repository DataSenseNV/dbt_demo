{{
	config(
		...
		alias='LKS_SALESFORCE_CAMPAIGNMEMBER_USER_LASTMODIFIEDBYID_TMP',
		schema='SALESFORCE_STG',
		tags=['SALESFORCE', 'LKS_SALESFORCE_CAMPAIGNMEMBER_USER_LASTMODIFIEDBYID_INCR']
	)
}}
select * from (
	WITH "DIST_STG" AS 
	...
	( 
		SELECT 
			  "STG_TEMP_SRC"."LNK_CAMPAIGNMEMBER_USER_LASTMODIFIEDBYID_HKEY" AS "LNK_CAMPAIGNMEMBER_USER_LASTMODIFIEDBYID_HKEY"
		...
		UNION 
		SELECT 
			  "LKS_SRC"."LNK_CAMPAIGNMEMBER_USER_LASTMODIFIEDBYID_HKEY" AS "LNK_CAMPAIGNMEMBER_USER_LASTMODIFIEDBYID_HKEY"
		...
		FROM {{ source('DEMO_FL', 'LKS_SALESFORCE_CAMPAIGNMEMBER_USER_LASTMODIFIEDBYID') }} "LKS_SRC"
		INNER JOIN {{ source('DEMO_FL', 'LNK_CAMPAIGNMEMBER_USER_LASTMODIFIEDBYID') }} "LNK_SRC" ON  "LKS_SRC"."LNK_CAMPAIGNMEMBER_USER_LASTMODIFIEDBYID_HKEY" = "LNK_SRC"."LNK_CAMPAIGNMEMBER_USER_LASTMODIFIEDBYID_HKEY"
	...
	SELECT 
		  "TEMP_TABLE_SET"."LNK_CAMPAIGNMEMBER_USER_LASTMODIFIEDBYID_HKEY" AS "LNK_CAMPAIGNMEMBER_USER_LASTMODIFIEDBYID_HKEY"
		...
		, CASE WHEN "TEMP_TABLE_SET"."SOURCE" = 'STG' AND TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."USER_LASTMODIFIEDBYID_HKEY")
			= LAG( TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."USER_LASTMODIFIEDBYID_HKEY"),1)OVER(PARTITION BY "TEMP_TABLE_SET"."CAMPAIGNMEMBER_HKEY" ORDER BY "TEMP_TABLE_SET"."LOAD_DATE","TEMP_TABLE_SET"."ORIGIN_ID")THEN 1 ELSE 0 END AS "EQUAL"

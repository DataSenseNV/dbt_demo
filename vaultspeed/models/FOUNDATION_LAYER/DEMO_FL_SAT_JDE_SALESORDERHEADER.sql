{{
	config(
		...
		schema='DEMO_FL',
		unique_key = ['"SALESORDERHEADER_HKEY"', '"LOAD_DATE"'],
		merge_update_columns = ['"LOAD_END_DATE"', '"DELETE_FLAG"', '"CDC_TIMESTAMP"'],
		tags=['JDEDWARDS', 'SAT_JDE_SOHEADER_INCR', 'SAT_JDE_SOHEADER_INIT']
	)
}}
select * from (
	WITH "END_DATING" AS 
	( 
		SELECT 
			  "SAT_TEMP_SRC_DK"."SALESORDERHEADER_HKEY" AS "SALESORDERHEADER_HKEY"
			, "SAT_TEMP_SRC_DK"."LOAD_DATE" AS "LOAD_DATE"
			, CASE WHEN LEAD( "SAT_TEMP_SRC_DK"."DELETE_FLAG" || "SAT_TEMP_SRC_DK"."SOURCE",1)OVER(PARTITION BY "SAT_TEMP_SRC_DK"."SALESORDERHEADER_HKEY" ORDER BY "SAT_TEMP_SRC_DK"."LOAD_DATE",
				"SAT_TEMP_SRC_DK"."LOAD_CYCLE_ID")= 'Y' || 'STG' THEN CAST('Y' AS VARCHAR) ELSE "SAT_TEMP_SRC_DK"."DELETE_FLAG" END AS "DELETE_FLAG"
			, LEAD("SAT_TEMP_SRC_DK"."CDC_TIMESTAMP",1)OVER(PARTITION BY "SAT_TEMP_SRC_DK"."SALESORDERHEADER_HKEY" ORDER BY "SAT_TEMP_SRC_DK"."LOAD_DATE") AS "CDC_TIMESTAMP"
		FROM {{ ref('JDEDWARDS_STG_SAT_JDE_SALESORDERHEADER_TMP') }} "SAT_TEMP_SRC_DK"
		WHERE  "SAT_TEMP_SRC_DK"."EQUAL" = 0
	)
	, "CALC_LOAD_END_DATE" AS 
	( 
		SELECT 
			  "SAT_TEMP_SRC_US"."SALESORDERHEADER_HKEY" AS "SALESORDERHEADER_HKEY"
			, "SAT_TEMP_SRC_US"."LOAD_DATE" AS "LOAD_DATE"
			, "SAT_TEMP_SRC_US"."LOAD_CYCLE_ID" AS "LOAD_CYCLE_ID"
			, COALESCE(LEAD("SAT_TEMP_SRC_US"."LOAD_DATE",1)OVER(PARTITION BY "SAT_TEMP_SRC_US"."SALESORDERHEADER_HKEY" ORDER BY "SAT_TEMP_SRC_US"."LOAD_DATE")
		...
		FROM {{ ref('JDEDWARDS_STG_SAT_JDE_SALESORDERHEADER_TMP') }} "SAT_TEMP_SRC_US"
		INNER JOIN "END_DATING" "END_DATING" ON  "SAT_TEMP_SRC_US"."SALESORDERHEADER_HKEY" = "END_DATING"."SALESORDERHEADER_HKEY" AND "SAT_TEMP_SRC_US"."LOAD_DATE" =
	...
	( 
		SELECT 
			  "CALC_LOAD_END_DATE"."SALESORDERHEADER_HKEY" AS "SALESORDERHEADER_HKEY"
	...
	SELECT 
		  "FILTER_LOAD_END_DATE"."SALESORDERHEADER_HKEY"
...
select * from (
	...
	( 
		SELECT 
			  "STG_INR_SRC"."SALESORDERHEADER_HKEY" AS "SALESORDERHEADER_HKEY"
	...
	SELECT 
		  "STG_SRC"."SALESORDERHEADER_HKEY" AS "SALESORDERHEADER_HKEY"

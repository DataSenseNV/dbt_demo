{{
	config(
		...
		alias='SAT_SALESFORCE_PRICEBOOK2_TMP',
		schema='SALESFORCE_STG',
		tags=['SALESFORCE', 'SAT_SALESFORCE_PRICEBOOK2_INCR']
	)
}}
select * from (
	WITH "DIST_STG" AS 
	...
	( 
		SELECT 
			...
			, "STG_TEMP_SRC"."ISSTANDARD" AS "ISSTANDARD"
		FROM {{ ref('SALESFORCE_STG_PRICEBOOK2') }} "STG_TEMP_SRC"
		WHERE  "STG_TEMP_SRC"."RECORD_TYPE" = 'S'
		UNION ALL 
		SELECT 
			  "SAT_SRC"."PRICEBOOK2_HKEY" AS "PRICEBOOK2_HKEY"
			...
			, "SAT_SRC"."ISSTANDARD" AS "ISSTANDARD"
		FROM {{ source('DEMO_FL', 'SAT_SALESFORCE_PRICEBOOK2') }} "SAT_SRC"
		INNER JOIN "DIST_STG" "DIST_STG" ON  "SAT_SRC"."PRICEBOOK2_HKEY" = "DIST_STG"."PRICEBOOK2_HKEY"
	...
	SELECT 
		...
		, CASE WHEN "TEMP_TABLE_SET"."SOURCE" = 'STG' AND TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."HASH_DIFF")
			= LAG( TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."HASH_DIFF"),1)OVER(PARTITION BY "TEMP_TABLE_SET"."PRICEBOOK2_HKEY" ORDER BY "TEMP_TABLE_SET"."LOAD_DATE","TEMP_TABLE_SET"."ORIGIN_ID")THEN 1 ELSE 0 END AS "EQUAL"
		...
		, "TEMP_TABLE_SET"."ISSTANDARD" AS "ISSTANDARD"

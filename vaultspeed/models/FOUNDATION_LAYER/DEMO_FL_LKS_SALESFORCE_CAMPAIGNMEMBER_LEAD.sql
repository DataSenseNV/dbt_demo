{{
	config(
		...
		schema='DEMO_FL',
		unique_key = ['"LNK_CAMPAIGNMEMBER_LEAD_HKEY"', '"LOAD_DATE"'],
		merge_update_columns = ['"LOAD_END_DATE"', '"DELETE_FLAG"', '"CDC_TIMESTAMP"'],
		tags=['SALESFORCE', 'LKS_SALESFORCE_CAMPAIGNMEMBER_LEAD_INCR', 'LKS_SALESFORCE_CAMPAIGNMEMBER_LEAD_INIT']
	)
}}
select * from (
	WITH "END_DATING" AS 
	( 
		...
		FROM {{ ref('SALESFORCE_STG_LKS_SALESFORCE_CAMPAIGNMEMBER_LEAD_TMP') }} "LKS_TEMP_SRC_DK"
		WHERE  "LKS_TEMP_SRC_DK"."EQUAL" = 0
	)
	, "CALC_LOAD_END_DATE" AS 
	( 
		SELECT 
			  "LKS_TEMP_SRC_US"."LNK_CAMPAIGNMEMBER_LEAD_HKEY" AS "LNK_CAMPAIGNMEMBER_LEAD_HKEY"
			...
			, "LKS_TEMP_SRC_US"."LEADID" AS "LEADID"
		FROM {{ ref('SALESFORCE_STG_LKS_SALESFORCE_CAMPAIGNMEMBER_LEAD_TMP') }} "LKS_TEMP_SRC_US"
	...
	( 
		SELECT 
			  "CALC_LOAD_END_DATE"."LNK_CAMPAIGNMEMBER_LEAD_HKEY" AS "LNK_CAMPAIGNMEMBER_LEAD_HKEY"
	...
	SELECT 
		  "FILTER_LOAD_END_DATE"."LNK_CAMPAIGNMEMBER_LEAD_HKEY"
...
select * from (
	...
	( 
		SELECT 
			  "STG_INR_SRC"."LNK_CAMPAIGNMEMBER_LEAD_HKEY" AS "LNK_CAMPAIGNMEMBER_LEAD_HKEY"
	...
	SELECT 
		  "STG_SRC"."LNK_CAMPAIGNMEMBER_LEAD_HKEY" AS "LNK_CAMPAIGNMEMBER_LEAD_HKEY"

{{
	config(
		...
		alias='LKS_JDE_SODETAIL_PRODUCT_TMP',
		schema='JDEDWARDS_STG',
		tags=['JDEDWARDS', 'LKS_JDE_SODETAIL_PRODUCT_INCR']
	)
}}
select * from (
	WITH "DIST_STG" AS 
	...
	( 
		SELECT 
			  "STG_TEMP_SRC"."LNK_SODETAIL_PRODUCT_HKEY" AS "LNK_SODETAIL_PRODUCT_HKEY"
		...
		UNION 
		SELECT 
			  "LKS_SRC"."LNK_SODETAIL_PRODUCT_HKEY" AS "LNK_SODETAIL_PRODUCT_HKEY"
		...
		FROM {{ source('DEMO_FL', 'LKS_JDE_SODETAIL_PRODUCT') }} "LKS_SRC"
		INNER JOIN {{ source('DEMO_FL', 'LNK_SODETAIL_PRODUCT') }} "LNK_SRC" ON  "LKS_SRC"."LNK_SODETAIL_PRODUCT_HKEY" = "LNK_SRC"."LNK_SODETAIL_PRODUCT_HKEY"
	...
	SELECT 
		  "TEMP_TABLE_SET"."LNK_SODETAIL_PRODUCT_HKEY" AS "LNK_SODETAIL_PRODUCT_HKEY"
		...
		, CASE WHEN "TEMP_TABLE_SET"."SOURCE" = 'STG' AND TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."PRODUCT_HKEY")
			= LAG( TO_CHAR("TEMP_TABLE_SET"."DELETE_FLAG") || TO_CHAR("TEMP_TABLE_SET"."PRODUCT_HKEY"),1)OVER(PARTITION BY "TEMP_TABLE_SET"."SALESORDERDETAIL_HKEY" ORDER BY "TEMP_TABLE_SET"."LOAD_DATE","TEMP_TABLE_SET"."ORIGIN_ID")THEN 1 ELSE 0 END AS "EQUAL"

{{
	config(
		...
		alias='LKS_SALESFORCE_CONTRACT_USER_OWNERID',
		schema='DEMO_FL',
		unique_key = ['"LNK_CONTRACT_USER_OWNERID_HKEY"', '"LOAD_DATE"'],
		merge_update_columns = ['"LOAD_END_DATE"', '"DELETE_FLAG"', '"CDC_TIMESTAMP"'],
		tags=['SALESFORCE', 'LKS_SALESFORCE_CONTRACT_USER_OWNERID_INCR', 'LKS_SALESFORCE_CONTRACT_USER_OWNERID_INIT']
	)
}}
select * from (
	WITH "END_DATING" AS 
	( 
		...
		FROM {{ ref('SALESFORCE_STG_LKS_SALESFORCE_CONTRACT_USER_OWNERID_TMP') }} "LKS_TEMP_SRC_DK"
		WHERE  "LKS_TEMP_SRC_DK"."EQUAL" = 0
	)
	, "CALC_LOAD_END_DATE" AS 
	( 
		SELECT 
			  "LKS_TEMP_SRC_US"."LNK_CONTRACT_USER_OWNERID_HKEY" AS "LNK_CONTRACT_USER_OWNERID_HKEY"
		...
		FROM {{ ref('SALESFORCE_STG_LKS_SALESFORCE_CONTRACT_USER_OWNERID_TMP') }} "LKS_TEMP_SRC_US"
	...
	( 
		SELECT 
			  "CALC_LOAD_END_DATE"."LNK_CONTRACT_USER_OWNERID_HKEY" AS "LNK_CONTRACT_USER_OWNERID_HKEY"
	...
	SELECT 
		  "FILTER_LOAD_END_DATE"."LNK_CONTRACT_USER_OWNERID_HKEY"
...
select * from (
	...
	( 
		SELECT 
			  "STG_INR_SRC"."LNK_CONTRACT_USER_OWNERID_HKEY" AS "LNK_CONTRACT_USER_OWNERID_HKEY"
	...
	SELECT 
		  "STG_SRC"."LNK_CONTRACT_USER_OWNERID_HKEY" AS "LNK_CONTRACT_USER_OWNERID_HKEY"
